...
$event_date_finish = $event_date_finish == ''?"0000-00-00":$event_date_finish;
	...
	$memberUpSelect = "UPDATE event SET 
	...
	$event_organizer = "UPDATE event_organizer SET 
						eo_name = '$eo_name',					    
	...
	mysqli_query($con,$event_organizer);
		...
		if ($_FILES['image']['name']<>'') {
		 	...
		 	delete_file('event_image','event','../images/events','event_id',$id)	;	
		 	mysqli_query($con,"UPDATE event SET event_image = '$nama_final' WHERE event_id = '$id' ");
		 	...
		 	if (isset($_POST['template_image'])) {
		 		...
		 		$query = "UPDATE event SET event_image = '$nama_image' WHERE event_id = '$id' " ;
